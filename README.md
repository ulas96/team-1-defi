...
| `script/DepositAndMint.s.sol` | Helper script. Optionally wraps AVAX, then deposits WAVAX collateral and mints SBT in a single broadcast.                 |
| `script/Burn.s.sol`         | Helper script. Approves the engine and burns SBT to reduce the caller's debt.                                               |
| `script/Liquidate.s.sol`    | Helper script. Approves the engine for the debt amount and liquidates an undercollateralized user.                          |
...
Deposits WAVAX collateral into the engine and mints SBT in one transaction. Optionally wraps native AVAX into WAVAX first.

| Variable            | Description                                                                      |
| ------------------- | -------------------------------------------------------------------------------- |
| `WAVAX_ADDRESS`     | Address of the WAVAX ERC-20 token                                                |
| `STABLETOKEN_ENGINE`    | Address of the deployed `StabletokenEngine`                                      |
| `STABLETOKEN`       | Address of the deployed `Stabletoken`                                            |
| `COLLATERAL_AMOUNT` | Amount of WAVAX (wei) to approve and deposit                                     |
| `MINT_AMOUNT`       | Amount of SBT (18 decimals) to mint against the deposit                          |
| `WRAP_AMOUNT`       | Optional. Amount of native AVAX (wei) to wrap into WAVAX before depositing       |

```bash
forge script script/DepositAndMint.s.sol:DepositAndMint \
  --rpc-url <RPC_URL> \
  --private-key <PRIVATE_KEY> \
  --broadcast
```
...
Burns SBT to pay down the caller's debt in the engine. The script approves the engine for `BURN_AMOUNT` and calls `engine.burn`.

| Variable         | Description                                     |
| ---------------- | ----------------------------------------------- |
| `STABLETOKEN_ENGINE` | Address of the deployed `StabletokenEngine`     |
| `STABLETOKEN`    | Address of the deployed `Stabletoken`           |
| `BURN_AMOUNT`    | Amount of SBT (18 decimals) to burn             |

```bash
forge script script/Burn.s.sol:Burn \
  --rpc-url <RPC_URL> \
  --private-key <PRIVATE_KEY> \
  --broadcast
```
...
Liquidates an undercollateralized user. The caller must hold and approve at least the user's full SBT debt; in exchange, the user's entire collateral balance is transferred to the caller.

| Variable         | Description                                                        |
| ---------------- | ------------------------------------------------------------------ |
| `STABLETOKEN_ENGINE` | Address of the deployed `StabletokenEngine`                        |
| `STABLETOKEN`    | Address of the deployed `Stabletoken`                              |
| `USER_ADDRESS`   | Address of the undercollateralized user to liquidate               |
| `SBT_AMOUNT`     | Amount of SBT (18 decimals) to approve — must cover the user's debt |
